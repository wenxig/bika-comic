...
<template>
  <div v-if="ctx.enable && ctx.ctx">
    <div v-if="!isInSmallestMode || isInAdmin"
      class="rounded-md shadow-lg fixed pointer-events-auto overflow-hidden transition-transform !duration-500"
      :class="[{ 'border-solid border-[--p-color] border-[3px]': isResizeing }, isInSmallestMode ? 'translate-x-[100%]' : 'translate-x-0']"
      ...
      <div class="absolute top-0 left-0 w-full h-[9%] van-hairline--bottom flex justify-center bg-[--van-background-3]"
      ...
      <div class="w-full h-[91%] absolute top-[9%] pointer-events-auto">
        <Swiper :modules="[Virtual, Zoom]" @swiper="sw => swiper = sw" :initialSlide
          ...
          <SwiperSlide v-for="(src, index) of ctx.ctx.images" :key="index" :virtualIndex="index" :data-hash="index + 1"
    ...
    <van-floating-bubble :gap="5" axis="xy" icon="chat" magnetic="x" v-model:offset="offset" @click="toNomralMode"
      v-if="isInSmallestMode"
      class="bg-[--van-background-3] border-[2px] border-solid border-[--van-border-color] shadow-lg pointer-events-auto transition-transform"
      teleport="#small-window">
