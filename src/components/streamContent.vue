...
<template>
  <!-- loading -->
  <div class="w-full flex justify-center items-center" v-if="stream.isRequesting.value"
    :class="[$props.class, classLoading]" :style="[style, styleLoading]">
    <VanLoading size="24px">加载中...</VanLoading>
  </div>

  <!-- retry -->
  <NResult v-else-if="stream.error.value" class="!items-center !justify-center flex flex-col" status="error"
    title="网络错误" :class="[$props.class, classError]" :style="[style, styleError]"
    :description="stream.error.value.message">
    <template #footer>
      <NButton @click="stream.retry()" type="primary">重试</NButton>
    </template>
    <template #icon>
  ...
  <!-- empty -->
  <NEmpty v-else-if="stream.isDone.value && stream.isEmpty.value" description="无结果" class="w-full !justify-center"
    :class="[$props.class, classEmpty]" :style="[style, styleEmpty]" />

  <!-- content -->
  <slot v-else :data="stream.data.value" />
</template>