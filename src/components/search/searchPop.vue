...
const searchMode = computed<SearchMode>(() => {
  if (inputText.value.startsWith(searchModeMap.uploader)) return 'uploader'
  if (inputText.value.startsWith(searchModeMap.translater)) return 'translater'
  if (inputText.value.startsWith(searchModeMap.anthor)) return 'anthor'
  if (inputText.value.startsWith(searchModeMap.id)) return 'id'
  if (inputText.value.startsWith(searchModeMap.tag)) return 'tag'
  if (inputText.value.startsWith(searchModeMap.categories)) return 'categories'
  return 'keyword'
})
