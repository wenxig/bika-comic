...
import { CommonComic, LessComic, spiltUsers, type RawCommonComic, type RawFullComic, type RawLessComic } from "../comic"
...
export namespace search {
  export type SearchResult<T = CommonComic> = RawStream<T>
  ...
  export const getComicsByTranslator = async (translator: string, page = 1, sort: SortType = 'dd', signal?: AbortSignal) => {
  ...
  export const getComicsByUploader = async (id: string, page = 1, sort: SortType = 'dd', signal?: AbortSignal) => createClassFromResponse(picapiRest.get<{ comics: SearchResult<RawLessComic> }>(`/comics?page=${page}&ca=${id}&s=${sort}`, { signal }), LessComic)

  export const getComicsByCategories = async (category: string, page = 1, sort: SortType = 'dd', signal?: AbortSignal) => createClassFromResponse(picapiRest.get<{ comics: SearchResult<RawLessComic> }>(`/comics?page=${page}&c=${encodeURIComponent(category)}&s=${sort}`, { signal }), LessComic)

  export const getComicsByTag = async (tag: string, page = 1, sort: SortType = 'dd', signal?: AbortSignal) => createClassFromResponse(picapiRest.get<{ comics: SearchResult<RawLessComic> }>(`/comics?page=${page}&t=${encodeURIComponent(tag)}&s=${sort}`, { signal }), LessComic)
