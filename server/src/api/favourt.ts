...
favourite.get('/image', async c => {
  const request = c.req
  const data = (<TypeOf<typeof putFavouriteImages>>await getWithChunk(`${request.param('uuid')}.favourite.image`)).filter(v => !isEmpty(v)) ?? []
  ...
  if (!isType(data, putFavouriteImages)) return util.typeError(c)
  const _data = putFavouriteImages.parse(data)
  const baseData: TypeOf<typeof putFavouriteImages> = (await getWithChunk(`${request.param('uuid')}.favourite.image`)) ?? []
